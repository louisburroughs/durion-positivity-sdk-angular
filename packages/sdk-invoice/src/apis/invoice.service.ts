...
export class InvoiceService extends BaseService {
    ...
    /**
     * Get Invoice Details
     * Returns the full invoice detail — status, line items, adjustments, totals, tax breakdown, due date and the resolved workorder number. Use this tool when the invoiceId is already known; use searchInvoices instead when locating an invoice by number, customer name or workorder number. Preconditions: the invoice must exist. A caller whose invoice:invoice:view grant is location-scoped must have the invoice\&#39;s location within reach (ADR-0061). Required inputs: invoiceId (UUID) as a path parameter; there is no request body. Emits an INVOICE_GET audit event; no state changes — this is a read-only projection. Returns 404 when no invoice exists for the supplied id, and 403 LOCATION_SCOPE_DENIED when the invoice exists but its location is outside the caller\&#39;s scope.
