...
export class PeopleAccessControlService extends BaseService {
     ...
     * Assigns a role to a person by delegating to pos-security through the person\&#39;s linked user account, bounded by an optional effective date window. Use this tool to grant access; do not use revokePersonRoleAssignment, which ends an assignment that already exists. Preconditions: the person must have an active user-person link resolving to a pos-security user, and the roleCode must exist in pos-security. Required inputs: personUuid (UUID) as a path parameter and roleCode in the body; optional startDate/endDate (ISO date-time) bound the assignment, with endDate required to be on or after startDate. The assignment takes no location. Per ADR-0061 it is an effective-dated user-to-role link and nothing more: the person\&#39;s location reach is the assigned role\&#39;s own location scope combined with that person\&#39;s pos-people staffing assignment, resolved at token issuance rather than chosen here. Emits a PEOPLE_CONTACT_ACCESS_ASSIGNMENT_CREATE event; the assignment record itself is created and owned by pos-security. Returns 201 with the created assignment, 400 when roleCode is blank or endDate precedes startDate, and 404 when the person\&#39;s user link, the security user, or the role cannot be resolved.
     * @endpoint post /v1/people/{personUuid}/access/assignments
     * @param personUuid
     * @param personRoleAssignmentRequest Role code plus an optional effective date window.
