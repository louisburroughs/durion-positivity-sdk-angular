...
export class JWTAPIService extends BaseService {
     ...
     * Exchanges a valid refresh token for a new access and refresh token pair, revoking the old pair in the same call (token rotation). Use this tool when an access token nears expiry and the client still holds a refresh token; do not use loginUser, which requires credentials, and do not use validateToken, which only checks a token without renewing it. Preconditions: the refresh token must be unexpired, unrevoked, present in the token store, and its user must still exist with at least one role and be enabled, unlocked, unexpired and holding unexpired credentials. Required inputs: refreshToken, the exact refresh token string previously issued. Emits a SECURITY_AUTH_REFRESH event, revokes the old access and refresh token JTIs in Redis, deletes the stored pair, and persists the replacement pair. Returns 400 when the refresh token is invalid, expired, revoked, or unknown; 401 with INVALID_REFRESH_TOKEN when the referenced user no longer exists; 401 with ACCOUNT_LOCKED, ACCOUNT_DISABLED, ACCOUNT_EXPIRED or CREDENTIALS_EXPIRED when the account\&#39;s state forbids the refresh (the same answers the credential login path gives, because a refresh-token holder is a credential-equivalent caller); 409 when concurrent refreshes race on the same token; and 403 with USER_HAS_NO_ROLES when the referenced user currently has no roles assigned — the token itself is valid, but the account holds no effective permissions until an administrator assigns a role.
