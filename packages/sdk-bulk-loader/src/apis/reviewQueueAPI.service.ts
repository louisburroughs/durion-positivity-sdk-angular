...
// @ts-ignore
import { AuditRecordResponse } from '../models/auditRecordResponse';
// @ts-ignore
import { BulkCorrectionItem } from '../models/bulkCorrectionItem';
// @ts-ignore
import { BulkCorrectionRequest } from '../models/bulkCorrectionRequest';
// @ts-ignore
import { BulkCorrectionResponse } from '../models/bulkCorrectionResponse';
// @ts-ignore
import { CorrectionResultDto } from '../models/correctionResultDto';
...
export class ReviewQueueAPIService extends BaseService {
     ...
     * @param options additional options
     */
    public listAuditRecords(jobId: string, observe?: 'body', reportProgress?: boolean, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<Array<AuditRecordResponse>>;
    public listAuditRecords(jobId: string, observe?: 'response', reportProgress?: boolean, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<HttpResponse<Array<AuditRecordResponse>>>;
    public listAuditRecords(jobId: string, observe?: 'events', reportProgress?: boolean, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<HttpEvent<Array<AuditRecordResponse>>>;
    public listAuditRecords(jobId: string, observe: any = 'body', reportProgress: boolean = false, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<any> {
        if (jobId === null || jobId === undefined) {
     ...
     * Submits corrected field values for one or more audit records of a FAILED or PARTIAL bulk load job, marking each accepted record CORRECTED. Use this tool to fix several failed rows in one call; use submitSingleCorrection instead when correcting exactly one record and a per-record accept or reject status is wanted. Preconditions: the job must belong to the authenticated operator and be in FAILED or PARTIAL state; each auditRecordId must belong to that job. Required inputs: corrections, a non-empty list where each item carries auditRecordId (UUID) and correctedData, a map of field names to corrected string values. Emits a BULK_LOADER_CORRECTION_SUBMIT event and stores the corrected values on each audit record; items whose audit record is missing or belongs to another job are rejected individually and reported in the response\&#39;s rejections list without failing the call. Correcting records does not re-run the import; call retryBulkLoadJob and then startJobProcessing to process the job again. Returns 201 with accepted and rejected counts, 409 when the job is not in FAILED or PARTIAL state, 404 when the job does not exist, and 403 when it belongs to another operator.
     ...
     * Submits corrected field values for exactly one audit record of a FAILED or PARTIAL bulk load job and reports whether the correction was accepted. Use this tool for interactive row-by-row fixing; use submitCorrections instead to correct a batch of records in one call. Preconditions: the job must belong to the authenticated operator and be in FAILED or PARTIAL state; the auditRecordId must belong to that job. Required inputs: auditRecordId (UUID) and correctedData, a map of field names to corrected string values. Emits a BULK_LOADER_CORRECTION_SUBMIT_SINGLE event and stores the corrected values on the audit record, setting its review status to CORRECTED when accepted; correcting a record does not re-run the import. Returns 201 with status ACCEPTED or REJECTED plus a rejectionReason when rejected. The response also carries the corrected audit record (entityType, entityId, rowNumber, reviewStatus, reasonCodes, originalValues, correctedValues and createdAt) so callers can update their review queue view without a follow-up fetch; these fields are omitted when the audit record can no longer be reloaded. Returns 409 when the job is not in FAILED or PARTIAL state, 404 when the job does not exist, and 403 when it belongs to another operator.
