...
export class BulkLoadJobsAPIService extends BaseService {
     ...
     * Creates a bulk load import job owned by the authenticated operator, starting in CREATED state with the target domain and expected file name recorded. Use this tool when starting a new bulk import from a file; do not use uploadJobFile, which attaches the file to a job that already exists, and do not use retryBulkLoadJob, which re-queues a FAILED or PARTIAL job. Preconditions: the operator must have no other job in an active state (CREATED, UPLOADING, DETECTING, MAPPING_REVIEW, DEDUP or PROCESSING); only one active job per operator is allowed. Required inputs: fileName (name of the source file that will be uploaded later) and domainType (see the domainType field\&#39;s schema below for the full, current set of import domains this endpoint accepts); locationId (UUID) is optional at creation but must be set before processing can start; tenantId (UUID) names the tenant the job loads into (ADR-0062): an active tenant of the cell, or the platform tenant, which accepts only SECURITY_ROLE and SECURITY_ROLE_PERMISSION (the role template\&#39;s roles.csv / role-permissions.csv); a bound caller may only name its own tenant, the platform operator included; the job, its audit rows and every call to a sibling service are bound to that tenant, and the job is visible afterwards only under that tenant\&#39;s binding. INVENTORY_STOCK_COUNT establishes opening on-hand stock: each line is filed and approved, so the token must carry inventory:adjustment:approve as well as inventory:adjustment:create. Emits a BULK_LOADER_JOB_CREATE event; no file content is stored by this call. Returns 201 with the new job; 400 with BULK_JOB_TENANT_REQUIRED when tenantId is omitted and no transitional default tenant is configured (with the default, the job uses it and logs a WARN); 400 with BULK_JOB_TENANT_UNKNOWN when the tenant is not active in this cell; 403 with BULK_JOB_TENANT_FORBIDDEN when a bound caller may not load into the tenant it named; 403 with BULK_JOB_TENANT_UNBOUND_TARGET_FORBIDDEN when the caller has no tenant bound on its token yet and still named one explicitly, rather than omitting tenantId for the transitional default; 403 with BULK_JOB_TENANT_DOMAIN_FORBIDDEN when the target is the platform tenant and domainType is not one of its own packs; 400 with BULK_JOB_DOMAIN_RETIRED when domainType is RETIRED; and 409 when the operator already has an active bulk load job in progress.
