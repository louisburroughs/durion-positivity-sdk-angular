...
export class BayAPIService extends BaseService {
     ...
     * Creates a service bay under a location with a type classification, concurrency capacity and optional capability and skill requirements. Use this tool when adding physical work capacity to a shop; do not use patchBay, which modifies a bay that already exists, and use createStorageLocation for inventory storage rather than vehicle bays. Preconditions: the location must exist, no bay of that location may already use the name (case-insensitive), and any serviceCapabilityCodes must name active catalog operation codes (a GENERAL_SERVICE bay declares none). Required inputs: name, bayType (one of GENERAL_SERVICE, ALIGNMENT, TIRE_SERVICE, HEAVY_DUTY, INSPECTION or WASH_DETAIL) and capacity.maxConcurrentVehicles of at least 1; status is optional, defaults to ACTIVE and only also accepts OUT_OF_SERVICE. Emits a LOCATION_BAY_CREATE event; no other records are touched. Returns 400 when locationId does not parse as a UUID, 403 LOCATION_SCOPE_DENIED when a location-scoped location:bay:manage grant does not cover locationId (ADR-0061), 404 when the location does not exist and 409 when the bay name is already taken at that location.
