...
export class PeopleStaffingAssignmentsService extends BaseService {
     ...
     * Creates a staffing assignment linking a person to a location with a role, effective dates, and a primary flag. Use this tool when a person starts working at a location; do not use updateStaffingAssignment, which modifies an existing assignment by id. Preconditions: the person must hold an ACTIVE employee record here, the location must be active, and no assignment for the same person, location, and role may overlap the effective dates. Required inputs: personId (UUID), locationId (UUID), role, isPrimary, and effectiveFrom (yyyy-MM-dd); effectiveTo is optional and open-ended when null. Location scope: an assignment feeds the person\&#39;s location-scope claims, so locationId must lie within the caller\&#39;s own location reach when their people:employee:edit permission is location-scoped, or the request is refused with 403 LOCATION_SCOPE_DENIED (checked after the person and location are resolved, so a 404 precedes it). Emits a PEOPLE_STAFFING_ASSIGNMENT_CREATE event and publishes a staffing-assignment fact; a new primary demotes and ends any overlapping existing primary, and a person\&#39;s first active assignment is forced primary regardless of the flag. Returns 409 when an overlapping assignment exists for the person, location, and role, 404 when the person, employee record, or active location cannot be resolved, 400 when the person\&#39;s employee status is not ACTIVE, and 403 LOCATION_SCOPE_DENIED when locationId is outside the caller\&#39;s location reach.
     ...
     * Ends a staffing assignment by setting its status to ENDED without physically deleting the row. Use this tool when a person stops working at a location; do not use disableEmployee, which offboards the whole employee, and do not use updateStaffingAssignment to shorten dates manually. Preconditions: the assignment must exist; ending an already ENDED assignment is accepted and republishes the fact. Required inputs: assignmentId (UUID) path parameter; there is no request body. Location scope: when the caller\&#39;s people:employee:edit permission is location-scoped, the assignment\&#39;s location must lie within the caller\&#39;s reach, or the request is refused with 403 LOCATION_SCOPE_DENIED (checked after the assignment is found, so a 404 precedes it). Emits a PEOPLE_STAFFING_ASSIGNMENT_END event and publishes a staffing-assignment fact; a null effectiveTo is stamped with today\&#39;s date. Returns 204 on success, 404 when no assignment exists for the supplied id, and 403 LOCATION_SCOPE_DENIED when the assignment\&#39;s location is outside the caller\&#39;s location reach.
