...
export class ApprovalConfigurationAPIService extends BaseService {
     ...
     * @param options additional options
     */
    public deleteApprovalConfiguration(approvalId: string, observe?: 'body', reportProgress?: boolean, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<any>;
    public deleteApprovalConfiguration(approvalId: string, observe?: 'response', reportProgress?: boolean, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<HttpResponse<any>>;
    public deleteApprovalConfiguration(approvalId: string, observe?: 'events', reportProgress?: boolean, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<HttpEvent<any>>;
    public deleteApprovalConfiguration(approvalId: string, observe: any = 'body', reportProgress: boolean = false, options?: {httpHeaderAccept?: 'application/json', context?: HttpContext, transferCache?: boolean}): Observable<any> {
     ...
     * Resolves the most specific approval configuration for a location and customer, trying the location-plus-customer row first, then the location-wide row, then the global default with no location or customer. Use this tool when deciding how an approval must be captured for a specific job; use getApprovalConfiguration instead when the configuration id is already known. Preconditions: both filters are optional and narrower matches win; a caller whose workorder:approval_config:view grant is location-scoped must have a supplied locationId within reach (ADR-0061). Required inputs: locationId (UUID) and customerId (UUID) as optional query parameters. No events are emitted and no state changes; this is a read-only projection. Returns 403 LOCATION_SCOPE_DENIED when the caller\&#39;s location scope does not cover the supplied locationId, and 404 when no configuration matches at any specificity, in which case callers fall back to built-in defaults.
